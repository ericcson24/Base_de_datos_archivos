...
type ShareModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onShare: (item: ShareItem, username: string, permission: string) => Promise<void>;
  onUnshareUser?: (item: ShareItem, username: string) => Promise<void>;
  item: ShareItem;
};

const ShareModal = ({ isOpen, onClose, onShare, onUnshareUser, item }: ShareModalProps) => {
  const { t } = useLanguage();
    ...
    const timeoutId = setTimeout(async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const filtered = (data.users || []).filter((u: ShareUser) =>
  ...
  const handleAddUser = (user: ShareUser) => {
  ...
  const handleRemoveSelected = (username: string) => {
  ...
  const handleShareGroup = async () => {
    ...
    try {
      ...
      setError((err as Error).message || t('share.error'));
    } finally {
      setLoading(false);
    }
  };

  const handleChangePermission = async (username: string, permission: string) => {
                  ...
                  <img
                    ...
                    onError={(e) => {
                      ...
                      if (el.dataset.fallback !== '1') {
                        ...
                        el.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(share.shared_with_username)}&background=random&size=36`;
                      }
                    }}
                  />

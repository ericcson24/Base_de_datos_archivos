...
interface EventModalProps {
  ...
  event?: any;
  mode: string;
  onSave: (eventData: any, mode: string) => Promise<void> | void;
  onDelete: (eventId: any) => Promise<void> | void;
  categories?: any[];
  isLoading?: boolean;
  selectedDates?: any;
  user?: any;
  initialAssignMode?: string;
...
const EventModal = ({
  ...
  const [dismissedSuggestions, setDismissedSuggestions] = useState<Set<any>>(new Set());
  const suggestionsTimerRef = useRef<any>(null);

  const loadAttachments = useCallback(async (eventId: any) => {
    if (!eventId) return;
    try {
  ...
  const fetchSuggestions = useCallback(async (eventTitle: string, eventDescription: string, eventLocation: string, eventCategories: any[]) => {
  ...
  const attachFile = async (file: any, eventId: any) => {
    try {
  ...
  const removeAttachment = async (attachmentId: any) => {
    try {
  ...
  const formatDateForDateInput = (date: any) => {
    if (!date) return '';
    const d = new Date(date);
    const offset = d.getTimezoneOffset() * 60000;
  ...
  const toggleCategory = (categoryName: string) => {
    setFormData((prev: any) => {
      ...
      if (currentCats.includes(categoryName)) {
        return { ...prev, categories: currentCats.filter((c: any) => c !== categoryName) };
      } else {
  ...
  const formatDateTime = (dateStr: any, allDay: any) => {
    if (!dateStr) return '';
  ...
  return (
    <>
                  ...
                  <div className="detail-group">
                    ...
                    <div className="detail-value">
                      <div className="categories-tags">
                        {event.extendedProps.categories.map((categoryName: string, index: number) => (
                          <span
                            key={index}
      ...
      <LocationPickerModal
        ...
        onSelect={(location: string) => handleInputChange('location', location)}

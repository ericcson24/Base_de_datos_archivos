...
type MoveModalProps = {
  ...
  onClose: () => void;
  itemToMove: MoveItem;
  onMove: (item: MoveItem, destination: string) => void;
};

const MoveModal = ({ isOpen, onClose, itemToMove, onMove }: MoveModalProps) => {
  const { t } = useLanguage();
  ...
  const loadFolders = async (path: string[]) => {
    ...
    try {
      ...
      const folderList = (data.files || []).filter((f: MoveFolder) => f.type === 'folder');

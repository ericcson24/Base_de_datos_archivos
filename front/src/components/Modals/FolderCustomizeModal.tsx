...
type FolderCustomizeModalProps = {
  isOpen: boolean;
  onClose: () => void;
  folder: CustomizeFolder;
  onSave: (folderPath: string, color: string, icon: string) => Promise<void> | void;
};

const FolderCustomizeModal = ({ isOpen, onClose, folder, onSave }: FolderCustomizeModalProps) => {
  const { t } = useLanguage();

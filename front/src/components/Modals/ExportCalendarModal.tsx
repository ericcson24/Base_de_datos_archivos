...
const ExportCalendarModal = ({ onClose }: ExportCalendarModalProps) => {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      sortedDates.forEach(date => {
        grouped[date].sort((a, b) => new Date(a.start || a.start_time || 0).getTime() - new Date(b.start || b.start_time || 0).getTime());
      });
  ...
  ${sortedDates.map(dateKey => `
    <div class="day-section">
      ...
      ${grouped[dateKey].map(ev => {
        ...
        let timeDisplay;
        if (isAllDay) {
          const startD = new Date(startTime as string);
          const endD = new Date(endTime as string);

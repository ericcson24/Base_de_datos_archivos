...
type SettingsModalProps = {
  ...
  isDarkMode: boolean;
  initialTab?: string;
  onUserUpdate?: (update: Partial<SettingsUser> & { language?: string; theme?: string }) => void;
  isCalendar?: boolean;
};
...
const SettingsModal = ({ onClose, user, onThemeToggle, isDarkMode, initialTab = 'general', onUserUpdate, isCalendar = false }: SettingsModalProps) => {
  ...
  const handleLinkMicrosoft = async () => {
    try {
      ...
      addToast(t('settings.connectionError', { error: (error as Error).message }), 'error');
    }
  };
                  ...
                  <div className="settings-avatar-wrapper">
                    <img
                      ...
                      onError={(e) => {
                        ...
                        img.onerror = null;
                        img.src = `https://ui-avatars.com/api/?name=${settings.username}&background=random`;
                      }}
                    />
                    <button
                    ...
                    <div className="space-y-6">
                        ...
                        {aiStatus.progress && aiStatus.progress.state === 'indexing' ? (
                            <div className="bg-white dark:bg-slate-700/50 p-4 rounded-lg border border-gray-200 dark:border-slate-600 shadow-sm animate-pulse">
                                ...
                                {aiStatus.progress.processed != null && aiStatus.progress.processed > 0 && (

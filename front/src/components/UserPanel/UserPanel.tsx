...
type PathEntry = { name: string; owner?: string };
...
const UserPanel = ({ user, onLogout, onBackToFolders, onThemeToggle, isDarkMode, onGoToCalendar, onGoToRemote, onUserUpdate }: UserPanelProps) => {
  ...
  const [rdpConnectionId, setRdpConnectionId] = useState<any>(null);
  const [showCustomizeModal, setShowCustomizeModal] = useState(false);
  const [customizeFolder, setCustomizeFolder] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    if (params.get('settings') === 'true') {
      ...
      if (params.get('tab')) {
        setSettingsInitialTab(params.get('tab') as string);
      }
  ...
  const [sidebarPanelFile, setSidebarPanelFile] = useState<any>(null);
  ...
  const xhrMapRef = useRef<Record<number, XMLHttpRequest>>({});
  ...
  const [editorPanels, setEditorPanels] = useState<any[]>([]);
  ...
  const [aiResultsData, setAIResultsData] = useState<any>(null);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (uploadMenuOpen) {
        ...
        if (uploadBtn && miniMenu && !uploadBtn.contains(event.target as Node) && !miniMenu.contains(event.target as Node)) {
      ...
      if (sharedDropdownOpen) {
        ...
        if (sharedBtn && dropdown && !sharedBtn.contains(event.target as Node) && !dropdown.contains(event.target as Node)) {
      ...
      if (isAIExpanded) {
        ...
        if (aiContainer && !aiContainer.contains(event.target as Node)) {
          setIsAIExpanded(false);
          setAIQuery('');
        }
      }
...
useEffect(() => {
  ...
  const handleFileUpload = useCallback(async (files: File[]) => {
    ...
    if (dupNames.length > 0) {
      duplicateAction = await new Promise<string | null>((resolve) => {
    ...
    const newUploads: UploadEntry[] = filesToUpload.map((file) => {
  ...
  const handleFolderUpload = useCallback(async (files: File[]) => {
    ...
    const newUploads: UploadEntry[] = Array.from(files).map((file) => {
      uploadIdRef.current += 1;
      return {
        id: uploadIdRef.current,
    ...
    for (const entry of newUploads) {
      await new Promise<void>((resolve) => {
        ...
        const formData = new FormData();
        formData.append('path', uploadPath);
        if ((entry._file as any).webkitRelativePath) {
          formData.append('relativePath', (entry._file as any).webkitRelativePath);
        }
  ...
  const handleCreateFile = useCallback(async (defaultName: string, type: string) => {
  ...
  const handleCreateFileConfirm = useCallback(async (fileName: string, type: string) => {
    if (!fileName) return;
  ...
  const handleRemoveShared = async (item: any) => {
    try {
  ...
  const handleToggleAIExclude = async (item: any) => {
  ...
  const handleUnshare = async (item: any) => {
    try {
      let allSuccess = true;
  ...
  const handleSaveToMyFiles = async (item: any) => {
    try {
  ...
  const handleUnpinFromPanel = async (item: any) => {
    try {
  ...
  const handlePinToPanel = async (item: any) => {
    try {
  ...
  const handleMoveItem = async (item: any, destinationPath: string) => {
    try {
  ...
  const handleShareItem = async (item: any, targetUsername: string, permission = 'edit') => {
    try {
  ...
  const handleUnshareUser = async (item: any, targetUsername: string) => {
    try {
      const response = await fetch('/api/files/unshare', {
        method: 'POST',
        headers: {
  ...
  const openCustomizeFolder = (item: any) => {
  ...
  const handleDropToFolder = useCallback(async (draggedFile: any, targetFolder: any) => {
    try {
  ...
  const handleBackBtnDragOver = useCallback((e: React.DragEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (e.dataTransfer.types.includes('application/x-internal-file')) {
  ...
  const handleBackBtnDragLeave = useCallback((e: React.DragEvent) => {
    e.preventDefault();
    e.stopPropagation();
    setBackBtnDragOver(false);
  }, []);

  const openSidebarPanel = (file: any, action = 'view') => {
  ...
  const openEditorPanel = (file: any) => {
  ...
  const closeEditorPanel = (panelId: number) => {
    setEditorPanels(editorPanels.filter(panel => panel.id !== panelId));
  };

  const bringPanelToFront = (panelId: number) => {
    const newZIndex = highestZIndex + 1;
  ...
  const handleFileDragStart = useCallback((file: any, e: React.DragEvent) => {
  ...
  {isSearchExpanded && (
    <>
      ...
      <AIResultsModal
        ...
        onOpenFile={(file: any) => openFileViewer(file)}
        onDownloadFile={(file: any) => downloadFile(file.id, file.name, t)}
      />

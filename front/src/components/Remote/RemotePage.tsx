...
const RemotePage = ({ user, onLogout, onGoBack, onGoToPanel, onGoToCalendar, onThemeToggle, isDarkMode }: RemotePageProps) => {
    const { t } = useLanguage();
    ...
    const handleConnectionClick = async (connectionId: any) => {
        const token = user?.token || getAuthToken();

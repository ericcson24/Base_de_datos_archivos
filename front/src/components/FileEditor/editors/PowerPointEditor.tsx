...
interface PowerPointEditorFile {
  id: string;
  name: string;
}
...
interface PowerPointEditorProps {
  fileUrl?: string;
  fileBlob?: Blob | null;
  file: PowerPointEditorFile;
  onClose?: () => void;
  onFileSaved?: () => void;
}

const PowerPointEditor: React.FC<PowerPointEditorProps> = ({ fileUrl, fileBlob, file, onFileSaved }) => {
  const { t } = useLanguage();
      ...
      try {
        ...
        while (zip.file(`ppt/slides/slide${slideIdx}.xml`)) {
          ...
          for (let s = 0; s < spNodes.length; s++) {
            ...
            let detectedAlign: string | null = null;
            let paraCount = 0;

            for (let p = 0; p < paragraphs.length; p++) {
              const para = paragraphs[p] as Element;
  ...
  const generatePptxBlob = async () => {
    ...
    slides.forEach((slide, idx) => {
      ...
      const algnMap: any = { left: 'l', center: 'ctr', right: 'r' };
      if (titleText) {
        shapes += `<p:sp>` +
  ...
  if (isPresentationMode) {
    ...
    const bgObj = SLIDE_BACKGROUNDS.find(b => b.id === slide.background) || SLIDE_BACKGROUNDS[0];
    const bgStyle: React.CSSProperties = bgObj.css.startsWith('linear') ? { background: bgObj.css } : { backgroundColor: bgObj.css };
    const contentLines = (slide.content || '').split('\n');
    ...
    return (
      <div className="ppt-presentation-mode" ref={containerRef} onClick={(e) => {
        ...
        <div className="ppt-presentation-slide" style={bgStyle}>
          <h1 style={{ color: slide.titleColor, textAlign: slide.titleAlign as any, fontSize: `${slide.titleSize * 1.5}px` }}>
            {slide.title}
          </h1>
          <div className="ppt-presentation-content" style={{ color: slide.contentColor, textAlign: slide.contentAlign as any, fontSize: `${slide.contentSize * 1.5}px` }}>
  ...
  const slideBgStyle: React.CSSProperties = bgObj.css.startsWith('linear') ? { background: bgObj.css } : { backgroundColor: bgObj.css };
        ...
        <div className="ppt-sidebar">
          <div className="ppt-sidebar-list">
            {slides.map((slide, index) => {
              ...
              const thumbBg: React.CSSProperties = bg.css.startsWith('linear') ? { background: bg.css } : { backgroundColor: bg.css };
              return (
                <div
                  key={index}
                  ...
                  <>
                    <input
                      ...
                      style={{
                        ...
                        textAlign: currentSlide.titleAlign as any,
                      }}
                    />
                    <textarea
                      ...
                      style={{
                        ...
                        textAlign: currentSlide.contentAlign as any,
                      }}
                    />
                  </>
                ) : (

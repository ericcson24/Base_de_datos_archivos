...
const ExcelEditor: React.FC<ExcelEditorProps> = ({ fileUrl, fileBlob, file, onFileSaved }) => {
  const { t } = useLanguage();
  ...
  const [mergedCells, setMergedCells] = useState<Record<string, any>>({});
  ...
  const [findMatches, setFindMatches] = useState<Cell[]>([]);
  ...
  const parseSheet = (sheet: any, wbParam?: any) => {
  ...
  const buildSheet = () => {
    ...
    if (Object.keys(mergedCells).length > 0) {
      const mergesList: any[] = [];
      Object.entries(mergedCells).forEach(([key, info]: [string, any]) => {
    ...
    Object.entries(cellStyles).forEach(([key, style]: [string, any]) => {
      const [r, c] = key.split('-').map(Number);
      const addr = XLSX.utils.encode_cell({ r, c });
      if (newSheet[addr]) {
        if (!(newSheet[addr] as any).s) (newSheet[addr] as any).s = {};
        if (style.fontWeight === 'bold') (newSheet[addr] as any).s.bold = true;
        if (style.fontStyle === 'italic') (newSheet[addr] as any).s.italic = true;
      }
    });
    return newSheet;
  };
  ...
  const getCellStyle = (rowIndex: number, colIndex: number): any => cellStyles[`${rowIndex}-${colIndex}`] || {};
  ...
  const handleApplyBorders = (type: string) => {
    if (!selectionRange) return;
  ...
  const sortColumn = (colIndex: number, ascending = true) => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const isFindBar = target.closest('.excel-find-bar');
      const isFormulaInput = target.classList?.contains('formula-input');
      ...
      const arrowMap: any = { ArrowUp: [-1, 0], ArrowDown: [1, 0], ArrowLeft: [0, -1], ArrowRight: [0, 1] };
      if (arrowMap[e.key]) {
        e.preventDefault();
        navigate(arrowMap[e.key][0], arrowMap[e.key][1], e.shiftKey);
        return;
      }
      ...
      <div className="excel-bottom-bar">
        <div className="sheet-tabs-bar">
          <div className="sheet-tabs">
            {workbook?.SheetNames.map((sheet: string) => (
              <button
                key={sheet}

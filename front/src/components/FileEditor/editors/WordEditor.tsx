...
const ReactQuill = dynamic(async () => {
  ...
  Wrapped.displayName = 'ReactQuillWrapper';
  return Wrapped;
}, { ssr: false }) as any;
...
function registerQuillFormats(Quill: any) {
  ...
  class StyledImage extends ImageBlot {
    static formats(domNode: any) {
      const formats = (super.formats ? super.formats(domNode) : {}) as any;
...
const WordEditor: React.FC<WordEditorProps> = ({ fileUrl, fileBlob, file, onFileSaved, highlightText }) => {
  const { t } = useLanguage();
  ...
  useEffect(() => {
    import('react-quill').then((mod: any) => {
      registerQuillFormats(mod.Quill || mod.default?.Quill);
      setQuillReady(true);
    });
  }, []);
    ...
    try {
      ...
      if (isZip) {
        ...
        try {
          ...
          let result = await mammoth.convertToHtml({ arrayBuffer }, options as any);
  ...
  useEffect(() => {
            ...
            img.onload = () => {
              ...
              setTimeout(() => {
                ...
                if (lastImg && !lastImg.getAttribute('width')) {
                  ...
                  lastImg.setAttribute('width', String(w));
                  lastImg.setAttribute('height', String(h));
    ...
    const deleteActiveImage = () => {
      ...
      if (!q) return;
      const blot = (window as any).Quill?.find(activeImg) || null;
      if (blot) {
    ...
    const updateOverlayPosition = () => {
      ...
      const scrollParent = quill.root.closest('.word-content') as HTMLElement | null;
    ...
    const showOverlay = (img: HTMLImageElement) => {
      ...
      corners.forEach(corner => {
        ...
        const cursors: any = { nw: 'nwse-resize', ne: 'nesw-resize', sw: 'nesw-resize', se: 'nwse-resize' };
        const positions: any = {
          nw: 'top:-5px;left:-5px;',
    ...
    const handleEditorClick = (e: MouseEvent) => {
      ...
      const img = target.closest ? target.closest('img') as HTMLImageElement | null : (target.tagName === 'IMG' ? target as HTMLImageElement : null);
    ...
    const handleDocClick = (e: MouseEvent) => {
      const target = e.target as HTMLElement;
      if (resizeOverlay && !resizeOverlay.contains(target)) {
  ...
  const generateDocxBlob = async () => {
    ...
    const esc = (str: any) => String(str || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');

    const parser = new DOMParser();
    const doc = parser.parseFromString(`<div>${htmlContent}</div>`, 'text/html');

    const images: Array<{ rId: string; data: Uint8Array; ext: string; cx: number; cy: number }> = [];
    ...
    for (let i = 0; i < imgNodes.length; i++) {
      ...
      if (src.startsWith('data:')) {
        await new Promise<void>((resolve) => {
          const img = new Image();
          img.onload = () => {
            node.setAttribute('data-real-width', String(img.width));
            node.setAttribute('data-real-height', String(img.height));
            resolve();
          };
          img.onerror = () => resolve();
          img.src = src;
    ...
    const getImageExt = (dataUri: string) => {
    ...
    const cssSizeToHalfPt = (size: string): string | null => {
      if (!size) return null;
    ...
    const cleanFontFamily = (font: string): string | null => {
      if (!font) return null;
    ...
    const runXml = (el: any): string => {
      ...
      el.childNodes.forEach((node: any) => {
        if (node.nodeType === Node.TEXT_NODE) {
          ...
          let fontSize = '24';
          let fontColor: string | null = isLink ? '0563C1' : null;
          if (isLink) underline = true;
          let bgColor: string | null = isCode ? 'f3f4f6' : null;
          let ancestor = el;
    ...
    const walkNodes = (container: any) => {
      container.childNodes.forEach((node: any) => {
        ...
        const headingMap: any = { h1: '1', h2: '2', h3: '3', h4: '4', h5: '5', h6: '6' };
        if (headingMap[tag]) {

...
const DailyTimeline = ({ events, headerActions }: { events: any[]; headerActions?: any }) => {
...
const Calendar = ({ user, onLogout, onBackToPanel, onBackToFolders, onGoToRemote, onThemeToggle, isDarkMode }: CalendarProps) => {
  const { t, language } = useLanguage();
  ...
  const [selectedCategories, setSelectedCategories] = useState<Set<any>>(new Set());
  ...
  const eventsCacheRef = useRef<Map<any, any>>(new Map());
  const loadedRangesRef = useRef<any[]>([]);
      ...
      const fetchData = async () => {
        try {
          ...
          if (groupsRes.ok) {
            ...
            if (data.success) {
              const groupsWithMembers = await Promise.all(data.groups.map(async (group: any) => {
  ...
  const handleUserSelect = (targetUserId: any) => {
  ...
  const [microsoftStatus, setMicrosoftStatus] = useState<any>({ linked: false, email: null });
  ...
  useEffect(() => {
    if (events.length > 0) {
      const eventCategories = new Set<any>();
      events.forEach(e => {
        const cats = e.categories || e.extendedProps?.categories;
        if (cats) {
          cats.forEach((c: any) => eventCategories.add(c));
        }
      });
      ...
      setCategories(prevCategories => {
        ...
        if (missing.length > 0) {
          const newCategories = missing.map(name => ({
            id: `generated-${(name as string).replace(/\s+/g, '-')}`,
            name: name,
            color: 'preset8',
            hexColor: stringToColor(name as string)
          }));
  ...
  const isRangeCovered = useCallback((start: number, end: number) => {
    return loadedRangesRef.current.some(r => r.start <= start && r.end >= end);
  }, []);

  const loadEvents = useCallback(async (startDate: any = null, endDate: any = null) => {
  ...
  const filteredEvents = events.filter(event => {
    ...
    return eventCats.some((category: any) => selectedCategories.has(category));
  }).map(event => {
  ...
  const updateEventDates = async (event: any, newStart: any, newEnd: any, revertFunc: any) => {
    try {
      const token = getAuthToken();
    ...
    <div className="calendar-layout">
      ...
      {modalState.isOpen && (
        <EventModal
          ...
          onSave={async (eventData: any, mode: string) => {
            try {
      ...
      {showRDPModal && (
        <RDPConnectionModal
          ...
          onConnect={() => setShowRDPModal(false)}
        />
      )}

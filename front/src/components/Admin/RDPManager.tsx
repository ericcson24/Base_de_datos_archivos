...
const RDPManager = () => {
    ...
    const [settings, setSettings] = useState<any>({ lan_only: 'false', server_id: '', maintenance_mode: 'false' });
    const [loading, setLoading] = useState(true);
    const [activeConnection, setActiveConnection] = useState<any>(null);
    const [defaultConnection, setDefaultConnection] = useState<any>(null);
    ...
    const toggleService = async (newState: boolean) => {
        try {
            const token = getAuthToken();
            const response = await fetch('/api/rdp/settings', {
                method: 'POST',
            ...
            if (response.ok) {
                setSettings((prev: any) => ({ ...prev, maintenance_mode: String(!newState) }));
                                              ...
                                              fetch('/api/rdp/settings', {
                                                ...
                                                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${getAuthToken()}` },
                                                 body: JSON.stringify({ lan_only: String(val) })
                                                  ...
                                                  setSettings((prev: any) => ({...prev, lan_only: String(val)}));
                                              }).catch(() => {
                                                  setSettings((prev: any) => ({...prev, lan_only: String(!val)}));
                                                  addToast(t('common.error'), 'error');
                                              });
